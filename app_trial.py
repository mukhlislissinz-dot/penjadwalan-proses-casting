...
st.set_page_config(
    page_title="Factory Scheduler V15 - GSheets Cloud",
    layout="wide")
...
def save_to_db(data):
    try:
        ...
        rows = [headers]
        for item in data:
            p_start = item['plan_start_pre'].isoformat() if isinstance(
                item['plan_start_pre'], datetime) else str(item['plan_start_pre'])
            end_post_str = "-"
            if item.get('fixed_data'):
                try:
                    # fixed_data may contain datetimes or ISO strings depending on source
                    if len(item['fixed_data']) >= 3:
                        end_dt = item['fixed_data'][2].get('Finish')
                    else:
                        end_dt = None
                    ...
                    end_post_str = "-"
            elif item.get('plan_start_pre') is not None:
                d_pre = float(item.get('d_pre', 12))
                cast = float(item.get('cast', 0))
                d_post = float(item.get('d_post', 33))
                padding = float(item.get('padding', 1))
                estimated_end = item['plan_start_pre'] + \
                    timedelta(hours=d_pre + cast + d_post + padding * 2)
                end_post_str = estimated_end.strftime("%H:%M (%d/%m)")

            def handler(x): return x.isoformat(
            ) if isinstance(x, datetime) else None
            fixed_json = json.dumps(item.get('fixed_data'), default=handler)
            rows.append(
                [
                    item.get(
                        'batch', ''), p_start, end_post_str, item.get(
                        'd_pre', 12), item.get(
                        'cast', 0), item.get(
                        'd_post', 33), str(
                            item.get(
                                'jit', True)), str(
                                    item.get(
                                        'locked', False)), item.get(
                                            'padding', 1), fixed_json])
        ws.update('A1', rows)
        ws.format("A:J", {"horizontalAlignment": "CENTER",
...
def save_settings(settings):
    try:
        ...
        st.error(f"Gagal simpan settings: {e}")
...
def save_finished_batches(data):
    try:
        ...
        rows = [headers]
        for item in data:
            rows.append([item.get("Nama Batch", ""), item.get(
                "Status Batch", ""), item.get("Status Proses", "")])
        ws.update('A1', rows)
...
def load_from_db():
    try:
        ...
        for r in rows[1:]:
            item = {
                'batch': r[0],
                'plan_start_pre': datetime.fromisoformat(
...
def load_finished_batches():
    try:
        ...
        if len(rows) <= 1:
            return []
        return [{"Nama Batch": r[0], "Status Batch": r[1],
                 "Status Proses": r[2]} for r in rows[1:]]
    except BaseException:
        return []
...
with st.sidebar:
    ...
    d_pre = st.number_input(
        "Durasi Pre-heat (Jam)",
        value=st.session_state.settings.get(
            'd_pre',
            12),
        key="sb_pre",
        on_change=update_settings_cb)
    default_cast = st.number_input(
        "Default Durasi Casting (Jam)",
        value=st.session_state.settings.get(
            'd_cast',
            9),
        key="sb_cast",
        on_change=update_settings_cb)
    d_post = st.number_input(
        "Durasi Post-heat (Jam)",
        value=st.session_state.settings.get(
            'd_post',
            33),
        key="sb_post",
        on_change=update_settings_cb)
    padding_val = st.number_input(
        "Padding (Jam)",
        value=st.session_state.settings.get(
            'padding',
            1),
        ...
        if st.button("🚀 Mulai Migrasi"):
            ...
            if os.path.exists(OLD_DB):
                ...
                try:
                    ...
                    conn.close()
                    if row_p:
                        save_to_db(json.loads(row_p[0]))
                    if row_s:
                        save_settings(json.loads(row_s[0]))
                    st.success("✅ Berhasil! Refresh halaman.")
                except Exception as e:
                    st.error(f"Gagal: {e}")
            else:
                st.warning("DB lokal tidak ada.")
...
with st.container(border=True):
    ...
    with c3:
        p_jam = st.time_input(
            ...
            disabled=jit_on)
    with c4:
        b_cast = st.number_input("Durasi Cast", value=default_cast)
...
if st.session_state.batch_list and mesin_sehat:
    ...
    p_starts = [item['plan_start_pre']
                for item in st.session_state.batch_list if not item.get('jit')]
    waktu_awal_simulasi = min(
        p_starts + [waktu_skrg]) if p_starts else waktu_skrg
    ...
    for idx, item in enumerate(st.session_state.batch_list):
        if 'd_pre' not in item:
            ...
            save_to_db(st.session_state.batch_list)
        c_pre, c_post, c_pad = item['d_pre'], item['d_post'], timedelta(
            hours=item['padding'])
        ...
        if item.get('locked') and item.get('fixed_data'):
            ...
            if item.get('jit'):
                saran_oven = sorted(
                    ketersediaan.items(),
                    key=lambda x: x[1])[0]
                start_pre, nama_bo_pre = max(
                    waktu_awal_simulasi, saran_oven[1]), saran_oven[0]
            else:
                start_pre = item['plan_start_pre']
                oven_ok = [m for m, t in ketersediaan.items() if t <=
                           start_pre]
                if not oven_ok:
                    continue
            ...
            bo_post_pool = {
                k: v for k, v in ketersediaan.items() if k in [
                    'Bo1', 'Bo2', 'Bo3', 'Bo4']}
            start_post_req = end_cast + c_pad
            oven_post_ready = [
                m for m, t in bo_post_pool.items() if t <= start_post_req]
            if not oven_post_ready:
                ...
                end_pre, start_pre = start_cast - \
                    c_pad, (start_cast - c_pad) - timedelta(hours=c_pre)
            else:
                start_post, nama_bo_post = start_post_req, oven_post_ready[0]
        ...
        for j in actual_job:
            if j['Mesin'] in ketersediaan:
                ketersediaan[j['Mesin']] = j['Finish']
            if j['Mesin'] == 'Casting Unit':
                casting_bebas = j['Finish']
        ...
        if status_kerja == "✅ Selesai":
            if not any(f['Nama Batch'] == item['batch']
                       for f in st.session_state.finished_list):
                st.session_state.finished_list.append(
                    {"Nama Batch": item['batch'], "Status Batch": "", "Status Proses": ""})
                save_finished_batches(st.session_state.finished_list)

        fmt = "%H:%M (%d/%m)"
        pre_range = f"{j_pre['Start'].strftime(fmt)} - {j_pre['Finish'].strftime(fmt)} [{j_pre['Mesin']}]"
        cast_range = f"{j_cast['Start'].strftime(fmt)} - {j_cast['Finish'].strftime(fmt)}"
        post_range = f"{j_post['Start'].strftime(fmt)} - {j_post['Finish'].strftime(fmt)} [{j_post['Mesin']}]"
        data_tabel.append({
            "Batch": item['batch'],
            "Status": status_kerja,
            "1. Pre-Heat (S|E|M)": pre_range,
            "2. Casting (S|E)": cast_range,
            "3. Post-Heat (S|E|M)": post_range,
        ...
        with st.expander(label):
            ...
            with c1:
                edit_name = st.text_input(
                    "Nama Batch", value=item['batch'], key=f"edit_n_{idx}")
            with c2:
                edit_jit = st.toggle(
                    "Autopilot", value=item.get(
                        'jit', True), key=f"edit_j_{idx}")
            c3, c4, c5 = st.columns([1.5, 1.5, 1])
            with c3:
                edit_tgl = st.date_input(
                    "Rencana Tgl Pre-heat",
                    value=item['plan_start_pre'].date(),
                    key=f"edit_d_{idx}",
                    disabled=edit_jit)
            with c4:
                edit_jam = st.time_input(
                    "Rencana Jam Pre-heat",
                    value=item['plan_start_pre'].time(),
                    key=f"edit_t_{idx}",
                    disabled=edit_jit)
            with c5:
                edit_cast = st.number_input(
                    "Durasi Cast", value=item.get(
                        'cast', 9), key=f"edit_c_{idx}")
            with st.container(border=True):
                st.caption("⚙️ Parameter Tersemat (Snapshot)")
                e_c1, e_c2, e_c3 = st.columns(3)
                with e_c1:
                    edit_pre = st.number_input(
                        "Durasi Pre-heat",
                        value=item.get(
                            'd_pre',
                            12),
                ...
                with e_c2:
                    edit_post = st.number_input(
                        ...
                        key=f"epost_{idx}")
                with e_c3:
                    edit_pad = st.number_input(
                        "Padding (Jam)", value=item.get(
                            'padding', 1), key=f"epad_{idx}")
            st.divider()
            ...
            with b1:
                if st.button(
                    ...
                    key=f"btn_s_{idx}",
                        use_container_width=True):
                    item.update({'batch': edit_name,
                                 'jit': edit_jit,
                                 'plan_start_pre': datetime.combine(edit_tgl,
                                                                    edit_jam),
                                 'cast': edit_cast,
                                 'd_pre': edit_pre,
                                 'd_post': edit_post,
            ...
            with b2:
                if not is_locked:
                    ...
                    if st.button(
                        ...
                        key=f"btn_u_{idx}",
                            use_container_width=True):
                        item.update({'locked': False, 'fixed_data': None})
                        save_to_db(st.session_state.batch_list)
                        st.rerun()
            with b3:
                if st.button(
...
if st.session_state.finished_list:
    ...
    st.info("Belum ada batch yang selesai.")
